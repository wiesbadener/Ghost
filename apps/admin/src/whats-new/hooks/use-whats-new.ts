...
export const useWhatsNew = (entries: ChangelogEntry[]): UseWhatsNewReturn => {
    ...
    // Initialize default settings on first load if user has no saved preferences
    useEffect(() => {
        if (!parsedSettings && isSuccess) {
            // Calculate default settings inline to avoid dependency issues
            const defaultSettings = getDefaultWhatsNewSettings();
            void updatePreferences({
                whatsNew: serializeWhatsNewSettings(defaultSettings),
            });
        }
    }, [parsedSettings, isSuccess, updatePreferences]);
